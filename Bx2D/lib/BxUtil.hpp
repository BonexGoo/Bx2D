...
namespace BxUtilGlobal
{
	...
	*/
	...
	/*!
	\brief 확장된 스트링길이(wchar) 구하기(\r과 \n도 인식)
	\param String : 스트링
	\return 스트링의 길이(wchar)
	*/
	static int StrLenEx(wstring String)
	{
		BxASSERT("BxUtil", String);
		int i = -1;
		while(String[++i] != _W2('\0') && String[i] != _W2('\r') && String[i] != _W2('\n'));
		return i;
	}

	/*!
	\brief 확장된 스트링길이(char) 구하기(사용자의 끝문자)
	\param String : 스트링
	\param ExitCode : 종료문자
	\return 스트링의 길이(char)
	*/
	...
	/*!
	\brief 확장된 스트링길이(wchar) 구하기(사용자의 끝문자)
	\param String : 스트링
	\param ExitCode : 종료문자
	\return 스트링의 길이(wchar)
	*/
	static int StrLenEx(wstring String, wchar ExitCode)
	{
		BxASSERT("BxUtil", String);
		int i = -1;
		while(String[++i] != _W2('\0') && String[i] != ExitCode);
		return i;
	}
	...
	*/
	static string_rw StrFree(string AllocedString)
	{
	...
	\param Data : 할당된 스트링
	*/
	static void StrFreeCB(void* Data)
	{
		BxCore::Util::Free((string_rw) Data);
	}
	...
	\param SrcLength : 복사될 사이즈(byte, SrcString의 길이로 자동셋팅 : -1)
	...
	*/
	...
	\param String : 스트링
	\param SrcLength : 스트링의 길이(byte, String의 길이로 자동셋팅 : -1)
	\return 해시코드
	*/
	...
	\param DstLength : 대상스트링의 길이(byte, DstString 길이로 자동셋팅 : -1)
	...
	*/
	static int StrSearch(string DstString, string SrcString, const int (&SrcSearchMap)[256], int DstLength = -1)
	{
		...
		while(DstFocus < DstLength)
		{
			...
			DstFocus += SrcSearchMap[DstString[DstFocus] & 0xFF];
		}
		return -1;
	}
	...
	*/
	...
	\param String : 스트링
	\param SrcLength : 스트링의 길이(byte, String 길이로 자동셋팅 : -1)
	...
	*/
	...
	/*!
	\brief 해당 스트링이 16진수인지의 여부
	\param String : 스트링
	\param SrcLength : 스트링의 길이(byte, String 길이로 자동셋팅 : -1)
	\return true-16진수, false-16진수아님
	*/
	static bool IsHexacode(string String, int StringLength = -1)
	{
		const int StringLen = (StringLength < 0)? StrLenEx(String) : StringLength;
		for(int i = 0; i < StringLen; ++i)
		{
			if('0' <= String[i] && String[i] <= '9') continue;
			else if('a' <= String[i] && String[i] <= 'f') continue;
			else if('A' <= String[i] && String[i] <= 'F') continue;
			return false;
		}
		return true;
	...
	\brief 스트링을 정수로 변환
	\param String : 스트링
	\param StringLength : 처리될 길이(byte, String의 길이로 자동셋팅 : -1)
	\return 변환된 정수
	*/
	...
	*/
	...
	\brief 헥사덤프를 양수로 변환
	\param Hex : 헥사덤프
	\param HexLength : 처리될 길이(byte, String의 길이로 자동셋팅 : -1)
	\return 변환된 양수
	*/
	...
	*/
	...
	/*!
	\brief 헥사덤프를 주소정수로 변환
	\param Hex : 헥사덤프
	\param HexLength : 처리될 길이(byte, String의 길이로 자동셋팅 : -1)
	\return 변환된 주소정수
	*/
	static mint HtoM(string Hex, int HexLength = -1)
	{
		mint MintValue = 0;
		const int StringLen = (HexLength < 0)? StrLenEx(Hex) : HexLength;
		for(int i = 0; i < StringLen; ++i)
	...
	/*!
	\brief 주소정수를 헥사덤프로 변환
	\param IsUpper : 대문자여부(true-대문자, false-소문자)
	\param Value : 주소정수
	\return 변환된 헥사덤프
	*/
	static string _tmp_ MtoH(bool IsUpper, mint Value)
	{
	...
	*/
	...
	/*!
	\brief 데이터에서 바이트배열 로드
	\param Resource : 읽을 데이터
	\param Dest : 쓸 데이터
	\param Length : 쓸 길이(byte)
	*/
...
public:
	...
	*/
	...
	\param Dst : 적용될 행렬
	*/
	global_func inline void MatrixClear(matrix _out_ Dst)
	{
	...
	*/
	...
	\param Src : 회전할 쿼터니언
	*/
	global_func inline void MatrixRotate(matrix _inout_ Dst, const quater _in_ Src)
	{
	...
	\param ResultE : 진출결과값
	*/
	global_func inline void GetCubicSpline(const fint VecX, const fint VecY,
		const fint VecXB, const fint VecYB, pointf _out_ ResultB,
		const fint VecXE, const fint VecYE, pointf _out_ ResultE)
	{
		...
		if(distb && (k = F_Divide_F(distb, dist)))
		{
			...
			ResultB.y = F_Divide_F(3 * (VecYB + F_Multiply_F(kk, VecY)), m);
		}
		...
		if(diste && (k = F_Divide_F(dist, diste)))
		{
			...
			ResultE.y = F_Divide_F(F_Multiply_F(k, 3 * (VecY + F_Multiply_F(kk, VecYE))), m);
		}
		else ResultE = pointf::zero();
	}
	...
	*/
	...
	\param cp949length : 변환할 CP949스트링 길이(byte, NULL문자제외, -1은 길이 자동측정)
	\param utf16 : 받을 UTF16스트링
	\param utf16length : 받을 UTF16스트링 길이(short, NULL문자제외)
	\return 변환된 배열수량
	*/
	...
	\param utf16length : 변환할 UTF16스트링 길이(short, NULL문자제외, -1은 길이 자동측정)
	\param cp949 : 받을 CP949스트링
	\param cp949length : 받을 CP949스트링 길이(byte, NULL문자제외)
	\return 변환된 배열수량
	*/
	...
	*/
	...
	\param utf8length : 변환할 UTF8스트링 길이(byte, NULL문자제외, -1은 길이 자동측정)
	\param utf16 : 받을 UTF16스트링
	...
	*/
	...
	/*!
	\brief UTF16을 UTF8로 전환시 필요한 UTF8의 길이(byte)
	\param utf16 : 변환할 UTF16스트링
	\param utf16length : 변환할 UTF16스트링 길이(short, -1은 길이 자동측정)
	\return 필요한 길이(byte)
	*/
	...
	/*!
	\brief UTF8을 UTF16으로 전환시 필요한 UTF16의 길이
	\param utf8 : 변환할 UTF8스트링
	\param utf8length : 변환할 UTF8스트링 길이(byte, -1은 길이 자동측정)
	\return 필요한 UTF16의 길이(short)
	*/
