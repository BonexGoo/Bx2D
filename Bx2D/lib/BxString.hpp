...
public:
	...
	public:
		...
		*/
		...
		{
			const int RHSCount = 1;
			if(StringLength <= ValidCount + RHSCount)
			{
				const int OldStringLength = StringLength;
				while((StringLength *= 2) <= ValidCount + RHSCount);
				string_rw NewString = BxUtilGlobal::StrAlloc(StringLength - 1);
		...
		\return 자신을 리턴
		*/
		inline Parse& operator+=(string RHS)
		{
			const int RHSCount = BxUtilGlobal::StrLen(RHS);
			if(StringLength <= ValidCount + RHSCount)
			{
				const int OldStringLength = StringLength;
				while((StringLength *= 2) <= ValidCount + RHSCount);
			...
			BxCore::Util::MemCpy(String + ValidCount, RHS, RHSCount);
			((char*) String)[ValidCount += RHSCount] = '\0';
			Text = BxCore::Font::TextClose(Text);
			return *this;
		}
		...
		*/
		...
		/*!
		\brief 형변환 중복함수(id_text)
		\return id_text형 전체스트링
		*/
		inline operator id_text() const
		{
			if(!Text)
		...
		*/
		...
		\param Src : 작업할 스트링
		\param Length : 작업할 스트링의 사이즈(byte, Other의 길이로 자동셋팅 : -1)
		*/
...
public:
	...
	\brief 기본생성자
	*/
	BxString() : Text(nullptr) {Chars[0] = '\0';}

	/*!
	\brief 복사생성자(BxString)
	\param RHS : 복사할 인스턴스
	*/
	BxString(const BxString& RHS) : Text(nullptr) {operator=(RHS);}

	/*!
	\brief 복사생성자(char)
	\param RHS : 복사할 인스턴스
	*/
	explicit BxString(char RHS) : Text(nullptr) {operator=(RHS);}

	/*!
	\brief 복사생성자(string)
	\param RHS : 복사할 인스턴스
	*/
	explicit BxString(string RHS) : Text(nullptr) {operator=(RHS);}

	/*!
	\brief 복사생성자(wstring)
	\param RHS : 복사할 인스턴스
	*/
	explicit BxString(wstring RHS) : Text(nullptr) {operator=(RHS);}
	...
	*/
	explicit BxString(string RHS, const BxArgument& Args) : Text(nullptr)
	{Format(RHS, Args);}
...
public:
	...
	*/
	...
	/*!
	\brief 연산자 중복함수=(wstring)
	\param RHS : 우측 피연산자
	\return 자신을 리턴
	*/
	BxString& operator=(wstring RHS)
	{
		Empty();
		char CP949[4096];
		BxUtil::UTF16ToCP949(RHS, -1, CP949, 4096);
		for(int i = 0; i < CP949[i]; ++i)
			Chars.Insert(i) = CP949[i];
		return *this;
	}
	...
	*/
	BxString& operator+=(wstring RHS)
	{
		char CP949[4096];
		BxUtil::UTF16ToCP949(RHS, -1, CP949, 4096);
		for(int i = 0; CP949[i]; ++i)
			Chars.Insert(END) = CP949[i];
		Text = BxCore::Font::TextClose(Text);
		return *this;
	}
	...
	*/
	...
	\brief 스트링으로 초기화
	\param Src : 스트링
	\param Length : 스트링의 길이(byte, -1은 길이 자동측정)
	*/
	...
	*/
	BxString& TrimBlank()
	{
		...
		{HasDelete = true; Chars.Delete(0);}
		while(1 < Chars.Length() && Chars[END - 1] == ' ')
		{HasDelete = true; Chars.Delete(END - 1);}
		if(HasDelete)
			Text = BxCore::Font::TextClose(Text);
		return *this;
	}
	...
	*/
	bool IsCharExtend(int Index) const
	{
	...
	*/
	...
	\return 배열위치, -1은 실패
	*/
	int ReverseFind(char Key) const
	{
		const int Count = Chars.Length() - 1;
	...
	\return string형 전체스트링
	*/
	inline string_rw CloneWithAlloc() const
	{
	...
	*/
	compare Compare(string Other) const
	{
	...
	\return same-같음, param_is_more-인수가 큼, param_is_less-인수가 작음
	*/
	compare Compare(const BxString& Other) const
	{
	...
	\return same-같음, param_is_more-인수가 큼, param_is_less-인수가 작음
	*/
	compare CompareNoCase(string Other) const
	{
		const int MinLen = BxUtilGlobal::Min(Chars.Length(), BxUtilGlobal::StrLen(Other) + 1);
		for(int i = 0; i < MinLen; ++i)
		{
	...
	\return same-같음, param_is_more-인수가 큼, param_is_less-인수가 작음
	*/
	compare CompareNoCase(const BxString& Other) const
	{
		const int MinLen = BxUtilGlobal::Min(Chars.Length(), Other.Chars.Length());
		for(int i = 0; i < MinLen; ++i)
		{
	...
	*/
	...
	\return 추출된 스트링
	*/
	inline BxString GetFileName() const
	{
	...
	\return 추출된 스트링
	*/
	inline BxString GetFilePath() const
	{
	...
	\return 추출된 스트링
	*/
	inline BxString GetFileTitle() const
	{
	...
	\return 추출된 스트링
	*/
	inline BxString GetFileExt() const
	{
...
protected:
	/// @cond SECTION_NAME
	mutable BxVar<char> Chars;
	mutable id_text Text;
	/// @endcond

	/// @cond SECTION_NAME
	int FindPathSlash() const
	{
		const int Count = Chars.Length() - 1;
		for(int i = 0; i < Count; ++i)
			if(Chars[i] == '/' || Chars[i] == '\\')
	...
	/// @cond SECTION_NAME
	int ReverseFindPathSlash() const
	{
		const int Count = Chars.Length() - 1;
		for(int i = Count - 1; 0 <= i; --i)
			if(Chars[i] == '/' || Chars[i] == '\\')
