...
public:
	...
	explicit ARG_HEX_UPPER(const int RHS) : Value((mint) RHS) {}
	explicit ARG_HEX_UPPER(const void* RHS) : Value((mint) RHS) {}
...
public:
	...
	explicit ARG_HEX_LOWER(const int RHS) : Value((mint) RHS) {}
	explicit ARG_HEX_LOWER(const void* RHS) : Value((mint) RHS) {}
...
public:
	...
	*/
	...
	/*!
	\brief 데이터길이(byte) 가져오기
	\return 데이터길이(byte)를 리턴
	*/
