...
class BxSingleton
{
	...
	public: template<typename TYPE>
	global_func inline TYPE* Rebind(const TYPE* value, const int newcount)
	{
		return List::Rebind(value, (0 < newcount)? new TYPE[newcount] : nullptr, newcount);
	}
	...
	class Value : public Element
	{
		...
		public: inline TYPE* Rebind(TYPE* newvalue, const int newcount)
		{return Transfer((TYPE**) ValuePtr, CountPtr, newvalue, newcount);}
	};
	...
	private: class List
	{
		...
		public: template<typename TYPE>
		global_func TYPE* Rebind(const TYPE* value, TYPE* newvalue, const int newcount)
		{
			List* CurList = Head();
			while(CurList = CurList->Next)
				if(CurList->Unit->IsSame(value))
					return ((Value<TYPE>*) CurList->Unit)->Rebind(newvalue, newcount);
			return nullptr;
		}
