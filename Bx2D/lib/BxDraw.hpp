...
#define XYS XYS
#define XYZS XYZS
#define XYARRAY XYARRAY
#define XYZARRAY XYZARRAY
#define XYSPLINE XYSPLINE
#define RGB32 RGB32
#define RGB16 RGB16
// 컬러링-스타일스택
#define CLIP CLIP
...
namespace BxDrawGlobal
{
	...
	\return 위치
	*/
	static inline const point XY(const int x, const int y)
	{
	...
	\return 크기
	*/
	static inline const size WH(const int w, const int h)
	{
	...
	*/
	...
	\return 영역
	*/
	static inline const rect XYR(const int x, const int y, BxDrawForm* form)
	{
	...
	*/
	...
	\return 다각영역
	*/
	static inline const points& XYARRAY(const uint count, const int* xy)
	{
	...
	\return 3D다각영역
	*/
	static inline const vertexs& XYZARRAY(const uint count, const int* xyz)
	{
	...
	\return 튜브영역
	*/
	static inline const spline& XYSPLINE(
	...
	{
		global_data spline Result = {{0,}, {0,}, {0,}, {0,}};
		Result.lx[0] = DtoF(lx0); Result.lx[1] = DtoF(lx1); Result.lx[2] = DtoF(lx2); Result.lx[3] = DtoF(lx3);
		Result.ly[0] = DtoF(ly0); Result.ly[1] = DtoF(ly1); Result.ly[2] = DtoF(ly2); Result.ly[3] = DtoF(ly3);
		Result.rx[0] = DtoF(rx0); Result.rx[1] = DtoF(rx1); Result.rx[2] = DtoF(rx2); Result.rx[3] = DtoF(rx3);
		Result.ry[0] = DtoF(ry0); Result.ry[1] = DtoF(ry1); Result.ry[2] = DtoF(ry2); Result.ry[3] = DtoF(ry3);
		return Result;
	}
	...
	*/
	...
	protected:
		...
		virtual bool _IsLoaded() {return true;}
	...
	*/
	static StyleStack CLIP(rect r, int hx = 0, int hy = 0)
	{
		...
		Stack().Clip[Focus].Offset.x = _DrawOption::CurHotspotX() = Stack().Clip[Focus - 1].Offset.x + ((r.l != -size::half)? r.l : 0) + hx; // 현재기준위치X
		Stack().Clip[Focus].Offset.y = _DrawOption::CurHotspotY() = Stack().Clip[Focus - 1].Offset.y + ((r.t != -size::half)? r.t : 0) + hy; // 현재기준위치Y
	...
	*/
	...
	{
		return CLIP(BxDrawGlobal::XYXY(-size::half, -size::half, size::half, size::half), hx, hy);
	}
	...
	*/
	static StyleStack COLOR(byte r, byte g, byte b, uint rate = 100)
	{
		...
		BxASSERT("BxDraw", Focus < Stack().MAX);
		const uint RateSum = BxUtilGlobal::Max(1, Stack().Color[Focus - 1].Sum + rate);
		Stack().Color[Focus].R = (Stack().Color[Focus - 1].R * Stack().Color[Focus - 1].Sum + r * rate) / RateSum;
		Stack().Color[Focus].G = (Stack().Color[Focus - 1].G * Stack().Color[Focus - 1].Sum + g * rate) / RateSum;
		Stack().Color[Focus].B = (Stack().Color[Focus - 1].B * Stack().Color[Focus - 1].Sum + b * rate) / RateSum;
	...
	*/
	static StyleStack FORM(BxDrawForm* form = nullptr, rect r = XYWH(0, 0, size::max, size::max))
	{
		...
		BxASSERT("BxDraw<로딩되지 않은 폼을 사용했습니다>", form->_IsLoaded());
...
public:
	...
	*/
	void Rectangle(BxDrawGlobal::option opt, rect r, BxDrawGlobal::StyleStack count = BxDrawGlobal::StyleStack::Zero())
	{
		...
		{
			Polygon(opt, BxDrawGlobal::XYS(BxARG(r.l, r.l, r.r, r.r), BxARG(r.t, r.b, r.b, r.t)), count);
			return;
		}
		// 모듈
		...
		if(0 < BxDrawGlobal::_DrawOption::CurAngle())
		{
            Polygon(opt, BxDrawGlobal::XYS(BxARG(r.l, r.r - 1, r.r - 1, r.l), BxARG(r.t, r.t, r.b, r.b)), count);
			return;
		}
		// 모듈
	...
	\param count : 스타일옵션, 다중설정시 >>사용
	*/
	void Rectangle9P(BxDrawForm* form, rect r, BxDrawGlobal::StyleStack count = BxDrawGlobal::StyleStack::Zero())
	{
		if(!VisibleTest(count)) return;
		// 모듈
		if(form && AddStyleByIf(count))
		{
			const int SEdgeX = form->Width() / 3, DEdgeX = BxUtilGlobal::Min(SEdgeX, (r.r - r.l) / 2);
			const int SEdgeY = form->Height() / 3, DEdgeY = BxUtilGlobal::Min(SEdgeY, (r.b - r.t) / 2);
			const int SX0 = 0, SX3 = form->Width(), SX1 = SX0 + SEdgeX, SX2 = SX3 - SEdgeX;
			const int SY0 = 0, SY3 = form->Height(), SY1 = SY0 + SEdgeY, SY2 = SY3 - SEdgeY;
			const int DX0 = r.l, DX3 = r.r, DX1 = DX0 + SEdgeX, DX2 = DX3 - SEdgeX;
			const int DY0 = r.t, DY3 = r.b, DY1 = DY0 + SEdgeY, DY2 = DY3 - SEdgeY;
			Rectangle(BxDrawGlobal::FILL, BxDrawGlobal::XYXY(DX0, DY0, DX1, DY1), BxDrawGlobal::FORM(form, BxDrawGlobal::XYXY(SX0, SY0, SX1, SY1)));
			Rectangle(BxDrawGlobal::FILL, BxDrawGlobal::XYXY(DX1, DY0, DX2, DY1), BxDrawGlobal::FORM(form, BxDrawGlobal::XYXY(SX1, SY0, SX2, SY1)));
			Rectangle(BxDrawGlobal::FILL, BxDrawGlobal::XYXY(DX2, DY0, DX3, DY1), BxDrawGlobal::FORM(form, BxDrawGlobal::XYXY(SX2, SY0, SX3, SY1)));
			Rectangle(BxDrawGlobal::FILL, BxDrawGlobal::XYXY(DX0, DY1, DX1, DY2), BxDrawGlobal::FORM(form, BxDrawGlobal::XYXY(SX0, SY1, SX1, SY2)));
			Rectangle(BxDrawGlobal::FILL, BxDrawGlobal::XYXY(DX1, DY1, DX2, DY2), BxDrawGlobal::FORM(form, BxDrawGlobal::XYXY(SX1, SY1, SX2, SY2)));
			Rectangle(BxDrawGlobal::FILL, BxDrawGlobal::XYXY(DX2, DY1, DX3, DY2), BxDrawGlobal::FORM(form, BxDrawGlobal::XYXY(SX2, SY1, SX3, SY2)));
			Rectangle(BxDrawGlobal::FILL, BxDrawGlobal::XYXY(DX0, DY2, DX1, DY3), BxDrawGlobal::FORM(form, BxDrawGlobal::XYXY(SX0, SY2, SX1, SY3)));
			Rectangle(BxDrawGlobal::FILL, BxDrawGlobal::XYXY(DX1, DY2, DX2, DY3), BxDrawGlobal::FORM(form, BxDrawGlobal::XYXY(SX1, SY2, SX2, SY3)));
			Rectangle(BxDrawGlobal::FILL, BxDrawGlobal::XYXY(DX2, DY2, DX3, DY3), BxDrawGlobal::FORM(form, BxDrawGlobal::XYXY(SX2, SY2, SX3, SY3)));
			SubStyle();
		}
	}
	...
	\param count : 스타일옵션, 다중설정시 >>사용
	*/
	void Polygon(BxDrawGlobal::option opt, points p, BxDrawGlobal::StyleStack count = BxDrawGlobal::StyleStack::Zero())
	{
		if(!VisibleTest(count)) return;
		#ifdef __BX_OPENGL
		// 모듈
	...
	*/
	...
	\brief 스플라인출력
	\param opt : FILL-채움그리기, DRAW-틀그리기
	\param t : 출력영역(XYSPLINE로 생성)
	\param count : 스타일옵션, 다중설정시 >>사용
	*/
	void Spline(BxDrawGlobal::option opt, const spline& s, BxDrawGlobal::StyleStack count = BxDrawGlobal::StyleStack::Zero())
	{
		if(!VisibleTest(count) || !AddStyleByIf(count))
			return;
		#ifdef __BX_OPENGL
			// 모듈
			id_opengl_outline Outline = BxCore::OpenGL2D::MakeOutline_SplineVertex(s);
			if(Outline)
			{
				const bool DoFlip = (BxDrawGlobal::_DrawOption::CurTurn() % 2 == 1);
				const int Angle = ((1024 << 16) + (BxDrawGlobal::_DrawOption::CurAngle() + BxDrawGlobal::_DrawOption::CurTurn() / 2 * 256)) % 1024;
	...
	\param align : 정렬방식
	\param count : 스타일옵션, 다중설정시 >>사용
	\return 마지막 출력된 글자의 우측하단
	*/
	const point TextS(id_font font, string str, const point pos, textalign align = textalign_left_top, BxDrawGlobal::StyleStack count = BxDrawGlobal::StyleStack::Zero())
	{
		id_text StrText = BxCore::Font::TextOpen(str);
		const point Result = Text(font, StrText, pos, align, count);
		StrText = BxCore::Font::TextClose(StrText);
		return Result;
	}
	...
	\return 마지막 출력된 글자의 우측하단
	*/
	const point TextW(id_font font, wstring wstr, const point pos, textalign align = textalign_left_top, BxDrawGlobal::StyleStack count = BxDrawGlobal::StyleStack::Zero())
	{
		id_text WStrText = BxCore::Font::TextOpenByUTF16(wstr, true);
		const point Result = Text(font, WStrText, pos, align, count);
		WStrText = BxCore::Font::TextClose(WStrText);
		return Result;
	}

	/*!
	\brief 영역방식 텍스트출력(id_text)
	\param font : 폰트ID
	\param text : 스트링(id_text)
	\param r : 출력영역
	\param align : 정렬방식
	\param count : 스타일옵션, 다중설정시 >>사용
	\return 마지막 출력된 글자의 우측하단
	*/
	const point TextByRect(id_font font, id_text text, const rect r, textalign align = textalign_left_top, BxDrawGlobal::StyleStack count = BxDrawGlobal::StyleStack::Zero())
	{
		point pos = point::zero();
		switch(align)
		{
		case textalign_left_top:      pos.x = r.l;             pos.y = r.t; break;
		case textalign_center_top:    pos.x = (r.l + r.r) / 2; pos.y = r.t; break;
		case textalign_right_top:     pos.x = r.r;             pos.y = r.t; break;
		case textalign_left_middle:   pos.x = r.l;             pos.y = (r.t + r.b) / 2; break;
		case textalign_center_middle: pos.x = (r.l + r.r) / 2; pos.y = (r.t + r.b) / 2; break;
		case textalign_right_middle:  pos.x = r.r;             pos.y = (r.t + r.b) / 2; break;
		case textalign_left_ascent:   pos.x = r.l;             pos.y = r.b; break;
		case textalign_center_ascent: pos.x = (r.l + r.r) / 2; pos.y = r.b; break;
		case textalign_right_ascent:  pos.x = r.r;             pos.y = r.b; break;
		case textalign_left_bottom:   pos.x = r.l;             pos.y = r.b; break;
		case textalign_center_bottom: pos.x = (r.l + r.r) / 2; pos.y = r.b; break;
		case textalign_right_bottom:  pos.x = r.r;             pos.y = r.b; break;
		}
		return Text(font, text, pos, align, count);
	}
	...
	\return 마지막 출력된 글자의 우측하단
	*/
	const point Text(id_font font, id_text text, const point pos, textalign align = textalign_left_top, BxDrawGlobal::StyleStack count = BxDrawGlobal::StyleStack::Zero())
	{
		if(!font || !text)
		{
			AddStyle(count);
		...
		if(!VisibleTest(count)) return point::zero();
		...
		point Result = BxCore::Font::DrawRequest(font,
			BxDrawGlobal::XY(pos.x + HotspotX - BxCore::Main::GetGUIMarginL(),
			pos.y + HotspotY - BxCore::Main::GetGUIMarginT()), text);
		BxCore::Font::RequestFlush(font);
...
public:
	...
	public:
		void SetEvent(string name, rect field)
		{
			...
			Field.l = field.l - BxCore::Main::GetGUIMarginL();
			Field.t = field.t - BxCore::Main::GetGUIMarginT();
			Field.r = field.r - BxCore::Main::GetGUIMarginL();
			Field.b = field.b - BxCore::Main::GetGUIMarginT();
		}
		void DeleteEvent()
		{
			Name[0] = '\0';
