...
namespace BxCore
{
	namespace AddOn
	{
		void* _inout_ ForExtend_JPGToBMP();
	}
}
local_func const byte* JPGToBMP(const byte* jpg);
local_func bool BindingCore() {BxCore::AddOn::ForExtend_JPGToBMP() = (void*) JPGToBMP; return true;}
...
static int DecompressImage(InputStructure*);

static int CleanUp(InputStructure *input)
{
	// Prevents Memory leaks	
	if (input->componentAttributes) BxFree(input->componentAttributes);
	if (input->out.streamY) BxFree(input->out.streamY);
	if (input->out.streamU) BxFree(input->out.streamU);
	if (input->out.streamV) BxFree(input->out.streamV);
...
static inline int markerSKIP(unschar **InPtr)
{
...
{
	const int args_scalefactor = 1;
	InputStructure _input = {0,};
	InputStructure* input = &_input;

	input->rawData = (unschar*) jpg;
	...
	input->multiScan = 0;
	input->DNL = 0;
	input->scalefactor = args_scalefactor;
	...
	////////////////////////////////////////////////////////////////////////////////
	const int Width = input->inputAttributes[0];
	const int Height = input->inputAttributes[1];
	const byte* Bmp = BxCore::AddOn::CreateBMP(Width, Height);
	bitmappixel* BmpFocus = BxCore::AddOn::GetBMPBits(Bmp) + (Width * (Height - 1));

	const byte* StreamYBegin = input->out.streamY;
	const byte* StreamUBegin = (input->nf == 3)? input->out.streamU : input->out.streamY;
	const byte* StreamVBegin = (input->nf == 3)? input->out.streamV : input->out.streamY;
	for(int y = 0; y < Height; ++y, BmpFocus -= Width)
	{
		const int StreamOffset = y * input->extwidth;
		const byte* StreamY = StreamYBegin + StreamOffset;
		const byte* StreamU = StreamUBegin + StreamOffset;
		const byte* StreamV = StreamVBegin + StreamOffset;
...
static int YUVtoRGB(InputStructure *input){
	...
	for (loop1=0;loop1<height;loop1+=input->scalefactor){
		for (loop2=0;loop2<width;loop2+=input->scalefactor){
			...
			yuv[1] = input->out.streamY[loop1*input->extwidth+loop2] - ((buffer_0_34414[input->out.streamU[loop1*input->extwidth+loop2]] + buffer_0_71414[input->out.streamV[loop1*input->extwidth+loop2]] + (1 << (rShift-1))) >> rShift);
...
static int SOF0(unschar **ipPtr, InputStructure *input){
	...
	if (input->DNL);							/* If DNL Marker is set by this time height information must also be read */
	else
		input->inputAttributes[1] = (**ipPtr * 256)+(*((*ipPtr)+1));	/*Height*/
	(*ipPtr) += 2;	
	...
	input->componentAttributes = (unschar*) BxAlloc(input->nf*6);
	memset(input->componentAttributes, 0, input->nf*6);
	if (!input->componentAttributes){		
		...
		input->out.streamY = (unschar*) BxAlloc(sizeof(char)*input->extheight*input->extwidth);
		...
		if (input->nf == 3){
			input->out.streamU = (unschar*) BxAlloc(sizeof(char)*input->extheight*input->extwidth);
			input->out.streamV = (unschar*) BxAlloc(sizeof(char)*input->extheight*input->extwidth);
