...
namespace BxCore
{
	namespace AddOn
	{
		void* _inout_ ForExtend_GIFToBMP();
	}
}
...
{
	(void) opaque;
	BxASSERT("BxCore::AddOn<NSGIF003의 오동작입니다>", bitmap);
}
...
const byte* GIFToBMP(const byte* gif, const int length, int _out_ numpage)
{
	...
	gif_animation gifani;
	gif_result code;

	// create our gif animation
	gif_create(&gifani, &bitmap_callbacks);
	...
	do
	{
		...
		{
			BxASSERT("BxCore::AddOn<NSGIF003의 모듈생성에 실패하였습니다>", false);
			return nullptr;
		}
	}
	while(code != GIF_OK);

	const int Width = gifani.width;
	const int Height = gifani.height;
	const int PageHeight = Height * gifani.frame_count;
	const byte* Bmp = BxCore::AddOn::CreateBMP(Width, PageHeight);

	// decode the frames
	bitmappixel* BmpFocus = BxCore::AddOn::GetBMPBits(Bmp);
	for(int i = 0, iend = gifani.frame_count; i < iend; ++i)
	{
		code = gif_decode_frame(&gifani, i);
		if(code != GIF_OK)
		{
			BxASSERT("BxCore::AddOn<NSGIF003의 디코딩에 실패하였습니다>", false);
			BxFree(Bmp);
			return nullptr;
		}
		const byte* ImgFocus = ((byte*) gifani.frame_image) + (Width * 4) * (Height - 1);
...
*/		gif_data += 3;
		...
		gif->global_colours = ((gif_data[4] & GIF_COLOUR_TABLE_MASK) != 0);
...
gif_result gif_decode_frame(gif_animation *gif, unsigned int frame) {
	...
	*/
	...
	frame_data = (unsigned int *)(void *)gif->bitmap_callbacks.bitmap_get_buffer(gif->frame_image);
	if (!frame_data)
		return GIF_INSUFFICIENT_MEMORY;
...
gif_decode_frame_exit:
	...
	*/
	...
	/*	Success!
	*/
	return (gif_result) return_value;
...
void gif_init_LZW(gif_animation *gif) {
	...
	gif->current_error = (gif_result) 0;
...
static bool gif_next_LZW(gif_animation *gif) {
	...
	if (code < 0) {
	  	gif->current_error = (gif_result) code;
		return false;
