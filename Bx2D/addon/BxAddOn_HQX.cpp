...
const byte* HQXToBMP(const byte* bmp, int scale)
{
	...
	// Source정보
	if(bmp[0] != '_' || bmp[1] != '_' || bmp[2] != 'B' || bmp[3] != 'M') return nullptr;
	bitmapinfo* SrcBmpInfoHeader = (bitmapinfo*)(bmp + 4 + sizeof(bitmapfile));
	...
	const byte* Bmp = BxCore::AddOn::CreateBMP(DstWidth, DstHeight);

	hqxInit();
	uint32_t* SrcBits = (uint32_t*) BxCore::AddOn::GetBMPBits(bmp);
	uint32_t* DstBits = (uint32_t*) BxCore::AddOn::GetBMPBits(Bmp);
	switch(scale)
	{
