...
namespace BxCore
{
	namespace AddOn
	{
		void* _inout_ ForExtend_BUFToMD5();
	}
}
local_func string _tmp_ BUFToMD5(const byte* buffer, const int length);
local_func bool BindingCore() {BxCore::AddOn::ForExtend_BUFToMD5() = (void*) BUFToMD5; return true;}
...
{
	static char Result[32 + 1] = {0,};
	MD5_CTX MD5;
	MD5Init(&MD5);
	MD5Update(&MD5, (byte*) buffer, length);
	MD5Final(&MD5);
	for(int i = 0; i < 16; ++i)
	{
		const byte DigestA = (MD5.digest[i] >> 4), DigestB = (MD5.digest[i] & 0x0F);
		Result[2 * i + 0] = (DigestA < 10)? '0' + DigestA : 'a' + DigestA - 10;
		Result[2 * i + 1] = (DigestB < 10)? '0' + DigestB : 'a' + DigestB - 10;
	}
	return Result;
}
// ■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
